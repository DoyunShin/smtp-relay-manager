"""Persistent identities, policies, credentials, and delivery metadata."""
...
class User(Base):
    ...
    username: Mapped[str] = mapped_column(
        String(64, collation="utf8mb4_bin"), unique=True
    )
...
class Domain(Base):
    ...
    active_name: Mapped[str | None] = mapped_column(
        String(253, collation="utf8mb4_bin"), unique=True
    )
...
class DomainAdmin(Base):
    ...
    domain_id: Mapped[str] = mapped_column(
        ForeignKey("domains.id"), primary_key=True
    )
...
class SMTPConfig(Base):
    ...
    security: Mapped[Literal["none", "starttls", "tls"]] = mapped_column(
        String(16)
    )
    auth: Mapped[Literal["none", "password"]] = mapped_column(String(16))
...
class CredentialScope(Base):
    ...
    credential_id: Mapped[str] = mapped_column(
        ForeignKey("smtp_credentials.id")
    )
    domain_id: Mapped[str] = mapped_column(ForeignKey("domains.id"))
    address: Mapped[str] = mapped_column(String(254, collation="utf8mb4_bin"))
    __table_args__ = (
        UniqueConstraint("credential_id", "domain_id", "address"),
    )
...
class SendAttempt(Base):
    ...
    domain_id: Mapped[str | None] = mapped_column(
        ForeignKey("domains.id"), index=True
    )
    sender: Mapped[str] = mapped_column(String(254))
    status: Mapped[Literal["pending", "accepted", "failed", "unknown"]] = (
        mapped_column(String(16), default="pending")
    )
...
class SendRecipient(Base):
    ...
    attempt_id: Mapped[str] = mapped_column(
        ForeignKey("send_attempts.id", ondelete="CASCADE"), index=True
    )

...
def _certificate_names(certificate: x509.Certificate) -> tuple[list[str], list[str]]:
    ...
    try:
        ...
        if common_names:
            dns_names.append(common_names[-1].value)
    else:
        dns_names.extend(san.get_values_for_type(x509.DNSName))
        ip_names.extend(str(value) for value in san.get_values_for_type(x509.IPAddress))
    ...
    try:
        ...
        for pattern in dns_names:
            ...
            if pattern == normalized:
                return
            if pattern.startswith("*.") and normalized.count(".") == pattern.count("."):
                if normalized.endswith(pattern[1:]):
                    return
    else:
        if any(ipaddress.ip_address(candidate) == address for candidate in ip_names):
            return
    raise CertificateError(f"certificate is not valid for {hostname}")


class CertificateManager:
    """Keep one SSLContext updated without interrupting established sessions."""

    def __init__(self, directory: Path, hostname: str) -> None:
        self.directory = directory
        self.hostname = hostname.rstrip(".").encode("idna").decode("ascii").lower()
    ...
    def usable(self) -> bool:
        ...
        return self._not_after is not None and self._not_after > datetime.now(UTC)
    ...
    def _check_sni(
        ...
        if server_name is not None and server_name.rstrip(".").lower() != self.hostname:
    ...
    def _read_files(self) -> tuple[bytes, bytes, bytes]:
        try:
            ...
            raise CertificateError("could not read SMTP certificate files") from exc
        fingerprint = hashlib.sha256(certificate + b"\x00" + private_key).digest()
        return certificate, private_key, fingerprint

    def reload(self, *, required: bool = False) -> bool:
        """Validate and activate changed files; preserve a valid old certificate."""

        try:
            certificate_bytes, private_key_bytes, fingerprint = self._read_files()
            ...
            if now < not_before or now >= not_after:
                raise CertificateError("SMTP certificate is not currently valid")
            ...
            with (
                tempfile.NamedTemporaryFile(prefix="smtp-cert-", suffix=".pem") as cert,
                tempfile.NamedTemporaryFile(prefix="smtp-key-", suffix=".pem") as key,
            ):
            ...
            if required or not self.usable:
                if isinstance(exc, CertificateError):
                    raise
                raise CertificateError("invalid SMTP certificate or private key") from exc
            LOGGER.error("SMTP certificate reload rejected (%s)", type(exc).__name__)
            return False
        self._fingerprint = fingerprint
        self._not_after = not_after
        self._active_context = candidate
        LOGGER.info("SMTP certificate loaded; expires at %s", not_after.isoformat())
        return True
